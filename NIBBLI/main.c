...
void main(void)
{
    ...
    while(1)
    {
        ...
        case WORKFLOW_INGAME:
            /* On détermine la direction dans laquelle veux aller le joueur */
            /* TODO : A modifier !! Le joueur veux une direction souhaitée mais pas sur */
            ...
            {
                /* A la position courante, on écrit le corps avec la direction */
                put_char(xt,yt,corps_direction[direction]);

                xt = xt + dirx;
                yt = yt + diry;

                /* Récupération du caractère à la position x,y de la tête */
                a = get_char(xt,yt);
                ...
                /* A la nouvelle position, on écrit la tete avec la direction */
                ...
                if (grossit==0)
                {
                    /* On efface la queue */
                    put_char(xq,yq,vide);
                    /* Ou se trouvera la prochaine queue à éteindre ? */
            ...
            {
                // Si il n'y à qu'une seule direction possible, on la choisit automatiquement !
                enable_nmi();
                delay(1);
                ...
                if (nbtest==1)
                {
                    ...
                    else if ((test3==32) || (test3==101))   // on va à gauche
                    {
                        direction=2;
                        dirx=-1;
                        diry=0;
                    }
                    else if ((test4==32) || (test4==101))   // on va à droite
                    {
                        direction=3;
                        dirx=1;
                        diry=0;
